-- ============================================================
-- place_order: validate, insert, auto-match (P2P price-time priority,
-- then Fund bid at Satış unlimited, then Fund ask at Alış capped by float).
-- All matched fills are 'pending' until İsmayıl confirms settlement.
-- ============================================================
...
-- Cancel an open/partial order (owner or admin); frees the share reservation.
...
begin
  ...
  if r.status not in ('open','partial') then
    raise exception 'order cannot be cancelled (status %)', r.status;
  end if;
  update public.orders set status='cancelled', remaining_units=0 where id = p_order_id;
  return json_build_object('ok', true);
end; $$;

revoke execute on function public.place_order(text,numeric,numeric) from public, anon;
