...
export function DebtNoticePanel({ debtors }: { debtors: Debtor[] }) {
  ...
  return (
    <section className="rounded-2xl border border-black/6 bg-white/90">
      <header className="flex items-center justify-between gap-3 px-5 py-4">
        ...
        {debtors.length > 0 && (
          <button
            ...
            disabled={busy !== null}
            className="shrink-0 rounded-full bg-[#2F61D8] px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.12em] text-white transition hover:bg-[#2854be] disabled:opacity-50"
          >
      ...
      {msg && <div className="px-5 pb-2 text-xs text-[#128342]">{msg}</div>}
      {error && <div className="px-5 pb-2 text-xs text-[#c74252]">{error}</div>}
      ...
      {debtors.length === 0 ? (
        ...
        <div className="divide-y divide-black/6 border-t border-black/6">
          {debtors.map((d) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-[#161616]">{d.name}</p>
              ...
              <button
                ...
                disabled={busy !== null}
                className="shrink-0 rounded-full border border-black/12 px-3 py-1.5 text-[11px] font-medium text-black/65 transition hover:border-[#2F61D8] hover:text-[#2F61D8] disabled:opacity-50"
              >
                {busy === d.name ? "..." : "Göndər"}
              </button>
            </div>
