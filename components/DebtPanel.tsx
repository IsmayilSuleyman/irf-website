  ...
  return (
    <div className="glass p-6 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div className="text-[10px] uppercase tracking-[0.22em] text-brand-red dark:text-red-400/80">
          Borclar
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
        <div>
          <div className="text-[11px] text-black/45 dark:text-white/50 mb-0.5">Ümumi Borc</div>
          <div className="num text-xl font-semibold text-black/85 dark:text-white/90">{formatAzn(totalRemaining)}</div>
          {totalOriginal > 0 && (
            <div className="text-[11px] text-black/45 dark:text-white/50 mt-0.5">
              {((totalRemaining / totalOriginal) * 100).toFixed(1)}% qalıb
            </div>
          )}
        </div>
        <div>
          <div className="text-[11px] text-black/45 dark:text-white/50 mb-0.5">Ödənilmiş</div>
          <div className="num text-xl font-semibold text-black/85 dark:text-white/90">{formatAzn(totalOriginal - totalRemaining)}</div>
          {totalOriginal > 0 && (
            <div className="text-[11px] text-black/45 dark:text-white/50 mt-0.5">
        ...
        <div>
          <div className="text-[11px] text-black/45 dark:text-white/50 mb-0.5">Aylıq Ödəniş</div>
          <div className="num text-xl font-semibold text-black/85 dark:text-white/90">{formatAzn(totalMonthly)}</div>
        </div>
      </div>
      ...
      {schedule.length > 1 && (
        <div>
          <div className="mb-3 text-[11px] text-black/45 dark:text-white/50">Borcun azalma proqnozu</div>
          <div className="h-52">
      ...
      {projections.length > 0 && (
        <div>
          <div className="mb-2 text-[11px] text-black/45 dark:text-white/50">Borcların siyahısı</div>
            ...
            {projections.map((debt) => (
              <li key={debt.name} className="py-3 flex flex-col gap-2">
                <div className="flex items-center justify-between gap-4">
                  <span className="text-sm text-black/85 dark:text-white/90 font-medium">{debt.name}</span>
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <span className="num text-sm font-semibold text-brand-red dark:text-red-400/80">
                      {formatAzn(debt.remainingAzn)}
                    </span>
                  </div>
                </div>
                <PaidOffBar remaining={debt.remainingAzn} original={debt.originalAzn} />
                <div className="flex items-center justify-between text-[11px] text-black/45 dark:text-white/50">
