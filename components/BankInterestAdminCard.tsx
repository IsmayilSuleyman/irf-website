...
import { settleBankInterest } from "@/app/bank/interest-actions";
import { formatGrouped } from "@/lib/portfolio";
import type { BankInterestHolderTotal } from "@/lib/bankInterest";

// İsmayıl's settlement view for the daily interest — the twin of the
// rewards card. Unsettled accruals count into the holder's displayed
// deposit; İsmayıl moves the amount into the Sheet deposit, then taps
// "Hesablaşdır" — the rows get a settled_at stamp and drop out of the
// displayed figure, so nothing double-counts.

export function BankInterestAdminCard({
  totals,
}: {
  totals: BankInterestHolderTotal[];
}) {
  ...
  const onSettle = (name: string, amountAzn: number) => {
    if (
      !window.confirm(
        `${name}: ${formatGrouped(amountAzn, 2)} ₼ faiz Sheet depozitinə köçürüldü? Təsdiqdən sonra bu məbləğ hesablaşmış sayılacaq.`,
      )
    ) {
      return;
    }
    ...
    startTransition(async () => {
      const res = await settleBankInterest(name);
      setBusyName(null);
      if (!res.ok) {
        setError(res.error);
        return;
  ...
  return (
    <div className="rounded-2xl border border-black/10 dark:border-white/10 bg-white/90 dark:bg-white/10 p-5">
          ...
          >
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] text-black/45 dark:text-white/50">
                <span className="num">{t.dayCount}</span> gün · cəmi{" "}
                <span className="num">{formatGrouped(t.totalAzn, 2)}</span> ₼
                {t.lastAccrual ? ` · son: ${t.lastAccrual}` : ""}
              </p>
            </div>
      ...
      <p className="mt-3 text-[11px] leading-5 text-black/45 dark:text-white/50">
        Faiz hər Bakı günü avtomatik hesablanır və növbəti gün holderin görünən
        depozitinə daxil olur. Məbləği Sheet depozitinə köçürəndən sonra
        "Hesablaşdır" bas — ikiqat sayılma olmasın deyə.
      </p>
    </div>
  );
}
