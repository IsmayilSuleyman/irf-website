...
function FactorDetail({ row, weights }: { row: ScoredItem; weights: MomentumWeights }) {
  const totalW =
  ...
  return (
    <div className="flex flex-col gap-1 pb-2 pl-14 pr-1 pt-1 text-[11px]">
      {FACTORS.map((f) => {
              ...
              <AnimatePresence initial={false}>
                {open && (
                  <motion.div
                    ...
                    className="overflow-hidden"
                  >
                    <FactorDetail row={row} weights={weights} />
                  </motion.div>
                )}
              </AnimatePresence>
            </li>
