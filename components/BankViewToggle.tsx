...
/**
 * Switch that flips the /bank page between the personal account view and the
 * whole-bank transparency view. Mirrors FundViewToggle: the mode is encoded in
 * the URL (`?view=bank`) so the server component re-renders with the right
 * dataset and the state survives refreshes and is shareable. /bank has no
 * other query params, so toggling off simply clears the search string.
 *
...
export function BankViewToggle({
  ...
  const toggle = () => {
    router.push(active ? pathname : `${pathname}?view=bank`, { scroll: false });
  };
